...
//
template <class Data, class FecGroup>
class HashSet
{
public:
   ...
   class iterator
   {
      friend class HashSet<Data, FecGroup>;
   ...
   public:
      ...
      Data* getObjAdr() { return &(_buckets[_row][_column]);}
   ...
   // if yes, return true;
   // else return false;
   bool check(const Data& d, size_t& mergeGate) const 
   { 
      ...
      for(size_t i = 0; i < _buckets[num].size(); i++)
      {
         if(_buckets[num][i] == d)
         {
            mergeGate = _buckets[num][i].getId();
            return true;
         }
      }
      return false; 
   ...
   bool checkGrp(const Data& d, FecGroup*& grp)
   {
      size_t num = bucketNum(d);
      for(size_t i = 0; i < _buckets[num].size(); i++){
         if(_buckets[num][i] == d){
            grp = _buckets[num][i].getFecGrp();
            return true;
         }
      }
      return false;
   }
   bool invCheckGrp(const Data& d, FecGroup*& grp)
   {
      size_t num = ~(d.getSimValue());
      num = num % _numBuckets;

      for(int i = 0; i < _buckets[num].size(); i++){
         if(_buckets[num][i] != d){       // overload != --> simValue = ~(k.simValue)
            grp = _buckets[num][i].getFecGrp();
            return true;
         }
      }
   ...
   void createNewGrp(Data& d)           // FecGroup = IdList
   {
      FecGroup* newGrp = new FecGroup;
      (*newGrp).push_back(d.getId());
      d.setFecGrp(newGrp);
   }
...
class HashKey
{
      // as hash function
public:
   HashKey(size_t a, size_t b, size_t c) : _in0(a), _in1(b), _id(c){}
   ~HashKey(){}
